...
class JacksonYamlResourceLoaderTest : JUnit5Minutests {
    fun tests() = rootContext {
        test("should successfully parse the job yaml configuration") {
            val subject: ObjectMapper = ObjectMapper(YAMLFactory()).disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES)
            val job = subject.readValue(File("src/main/resources/io/armory/plugin/preconfigured/job.yaml"), KubernetesPreconfiguredJobProperties::class.java)
